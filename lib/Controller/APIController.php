...
class APIController extends OCSController {
	...
	public function editRetention(int $id, int $timeunit, int $timeamount, int $timeafter = Constants::CTIME): DataResponse {
		if (($timeunit < 0 || $timeunit > 3) ||
			($timeamount < 1) ||
			($timeafter < 0 || $timeafter > 1)) {
			return new DataResponse([], Http::STATUS_BAD_REQUEST);
		}
		...
		$qb = $this->db->getQueryBuilder();
		$qb->update('retention')
			->set('time_unit', $qb->createNamedParameter($timeunit))
			->set('time_amount', $qb->createNamedParameter($timeamount))
			->set('time_after', $qb->createNamedParameter($timeafter))
			->where($qb->expr()->eq('id', $qb->createNamedParameter($id)));
		$qb->executeStatement();
